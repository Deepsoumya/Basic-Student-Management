...
{
 // as the method type in the form is "post"
 ...
 $query=mysqli_query($db,"SELECT * FROM students WHERE email='$inEmail'");
 $numrows=mysqli_num_rows($query);
 if($numrows==0)
 {

  /*$stmt = $db->prepare("INSERT INTO students(email, firstname, password,lastname,city,phone) VALUES(?, ?, ?, ?, ?, ?)"); //Fetching all the records with input credentials
  //if ( !$stmt ) {
    //printf('errno: %d, error: %s', $mysqli->errno, $mysqli->error);
    //die;
//}
  $stmt->bind_param("ssssss", $inEmail, $inFirstname, $inPassword, $inLastname, $inCity, $inPhone); //Where s indicates string type. You can use i-integer, d-double
  $stmt->execute();
  $result = $stmt->affected_rows;
  $stmt -> close();
  $db -> close();*/
  $sql = "INSERT INTO students (email, fname, password,lname,address,age) VALUES ('$inEmail', '$inFirstname', '$inPassword', '$inLastname', '$inCity', '$inPhone')";
  if(mysqli_query($db, $sql))
  {
  //header("location: successful.php"); // user will be taken to the success page
  echo "<center><p style='color:white'>You are successfully registered.";
  ...
  echo "<center><p style='color:white'>email already exists, try something new.</p></cemter>";
}
}else {
      ...
      .connection-main{
        ...
        <div class="col-lg-6 col-md-8 login-box">
            ...
            <div class="col-lg-12 col-md-12" style="color:white;">
              ...
              <h6 class="connection-login">Already have an account? <a href="login.php" class="login-here" style="text-decoration:none;">LogIn here</a></h6><br><br><br>
            </div>
        </div>
    </div>
</body>
