...
if(!empty($_POST['username_log']) && !empty($_POST['password_log'])) {
    ...
    $query=mysqli_query($db,"SELECT * FROM students WHERE email='$user' AND password='$pass'");
    ...
    /* Redirect browser */
    if(isset($_POST['username_log']) && !empty($_POST['username_log']) && isset($_POST['password_log']) && !empty($_POST['password_log'])){
    //$username = mysqli_escape_string($db,$_POST['nickname_log']);
    //$password = mysqli_escape_string($db,md5($_POST['password_log']));

    $search1 = mysqli_query($db,"SELECT email,password FROM students WHERE email='$user' AND password='$pass'") or die(mysqli_error($db));
    $match1  = mysqli_num_rows($search1);
    if($match1 != 0){
      $_SESSION['sess_user']=$user;
      $_SESSION['sess_id']=$id;
      $_SESSION['sess_name']=$name;
      $_SESSION['sess_courseId']=$courseId;
      $_SESSION['fname0']=$fname0;
                    ...
                    <div class="col-lg-12 login-form">
                        <form method="post" action="">
                            <div class="form-group">
                                ...
                                <input type="text" name="username_log" class="form-control" required>
                            </div>
                            <div class="form-group">
